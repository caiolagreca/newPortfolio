...
const ProExp = () => {
  ...
  const parseDateString = (dateString: string | undefined) => {
    ...
    const parsedDate = parse(dateString, 'dd/MM/yyyy', new Date(), { locale: enUS });
                ...
                {experiences.length > 0 && experiences[activeIndex] && (
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-500 mb-2">
                      {formatDate(parseDateString(experiences[activeIndex].startDate))} –{' '}
                      {experiences[activeIndex].isCurrent
                        ? 'Present'
                        : formatDate(parseDateString(experiences[activeIndex].endDate))}
                    </p>
